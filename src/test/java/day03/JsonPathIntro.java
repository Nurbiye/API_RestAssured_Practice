...
// and save it into variable of correct type
      ...
      // response.prettyPrint();
        ...
        int myId =  jp.getInt("id"); //we want to store and reuse it
    ...
    @DisplayName("Extracting data from Json Array Response")  //ex: get all spartan
    @Test
    public void getAllSpartanExtractData() {

        Response response = get("/spartans");  //this gives response object
        JsonPath jp = response.jsonPath();
        ...
        //loop? for whole ..? no w=neet, we have js.getList method
        //json is used groovy language, and it is simple
        System.out.println("jp.getString(\"name[0]\") = " + jp.getString("name[-1]"));//get last one
        System.out.println("jp.getString(\"name[0]\") = " + jp.getString("name[1..5]")); //get 1 to 5th object


        System.out.println("jp.getLong(\"phone[0]\") = " + jp.getLong("phone[0]"));

        //get the number 7th json object gender field from json array
        System.out.println("jp.getString(\"gender[6]\") = " + jp.getString("gender[6]"));


        //getting all the name fields from jsonArray Response, and storing as a List
        List<String> allNames = jp.getList("name");
        System.out.println("allNames = " + allNames);


        //getting all the phone fields from jsonArray Response, and storing as a List
...
// send request to this request url
    ...
    // print it out
     ...
     JsonPath jp = given()
             ...
             when()
             .get("/spartans/search")    //this is where we can get response object
             .jsonPath();
        ...
        //given --> all the extra information that we can give
    //when is the place we send our request
